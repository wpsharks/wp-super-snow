...
class plugin
{
    ...
    {
        return !empty($_REQUEST[__NAMESPACE__.'_pro_preview']);
    }
    ...
    public function forecasts_snow()
    {
        if (!$this->options['enable']) {
            $snow = false;
        } elseif ($this->is_multisite_farm_blog() && wp_is_mobile()) {
            $snow = false;
        } elseif (!$this->is_multisite_farm_blog() && $this->options['conditionals']
    ...
    public function enqueue_scripts()
    {
        ...
        wp_enqueue_script(__NAMESPACE__, $this->url('/client-s/js/wp-super-snow.min.js'), array('jquery'), $this->version, true);

        $flakes = preg_split('/['."\r\n".']+/', $_this->options['flakes'], -1, PREG_SPLIT_NO_EMPTY);
    ...
    public function enqueue_admin_styles()
    {
        ...
        wp_enqueue_style(__NAMESPACE__, $this->url('/client-s/css/menu-pages.min.css'), array(), $this->version, 'all');
    }
    ...
    public function enqueue_admin_scripts()
    {
        ...
        wp_enqueue_script(__NAMESPACE__.'-snow', $this->url('/client-s/js/wp-super-snow.min.js'), array('jquery'), $this->version, true);
        wp_enqueue_script(__NAMESPACE__, $this->url('/client-s/js/menu-pages.min.js'), array('jquery'), $this->version, true);
    }
    ...
    {
        if (!is_array($errors = get_option(__NAMESPACE__.'_errors'))) {
