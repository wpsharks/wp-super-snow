...
class menu_pages
{
    public function options()
    {
        ...
        if (!empty($_REQUEST[__NAMESPACE__.'__updated'])) {
        ...
        if (!empty($_REQUEST[__NAMESPACE__.'__restored'])) {
            echo '<div class="plugin-menu-page-notice notice">'."\n";
        ...
        echo '                  <p>'.__('If you\'d like to add your own images; or replace the <a href="'.esc_attr(plugin()->url('/client-s/images/default-snowflake-graphics.zip')).'">default snowflake graphics</a> — you can do that here. We recommend alpha transparent PNG files approx 100 x 100 pixels. There is no limit on the number of graphics (one image path per line please).', plugin()->text_domain).'</p>'."\n";
